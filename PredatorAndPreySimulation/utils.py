...
def PreyFilterUsingEuclideanDistances(Position, ListOfCounterCreatures, Upperbound):
    ...
    for animal in ListOfCounterCreatures:
        ...
        vectorx_y = pygame.math.Vector2(animal.rect.centerx - Position[0], animal.rect.centery - Position[1])
...
def triangle(x, y):
    triangle_center_x, triangle_center_y = x, y
    triangle_height = math.sqrt(3)/2 * triangle_side_length
    triangle_top_x, triangle_top_y = triangle_center_x, triangle_center_y - triangle_height/3*2
    triangle_left_x, triangle_left_y = triangle_center_x - triangle_side_length/2, triangle_center_y + triangle_height/3
    triangle_right_x, triangle_right_y = triangle_center_x + triangle_side_length/2, triangle_center_y + triangle_height/3

    # 计算向量的 X 和 Y 分量
    vector_x = vector_magnitude * math.cos(math.radians(vector_angle))
    vector_y = vector_magnitude * math.sin(math.radians(vector_angle))

    # 移动三角形
    triangle_center_x += vector_x
    triangle_center_y += vector_y
    triangle_top_x += vector_x
    triangle_top_y += vector_y
    triangle_left_x += vector_x
    triangle_left_y += vector_y
    triangle_right_x += vector_x
    return triangle_color,triangle_left_x,triangle_left_y,triangle_right_x,triangle_right_y,triangle_top_x,triangle_top_y
